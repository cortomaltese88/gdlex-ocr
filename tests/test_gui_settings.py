...
class GuiSettingsPersistenceTest(unittest.TestCase):
    ...
    def test_restores_saved_gui_options_without_input_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            settings.setValue("judgment/analyzeAfterConversion", True)
            ...
            self.assertTrue(window.judgment_analysis_checkbox.isChecked())
    ...
    def test_saves_relevant_options_and_never_saves_pdf_input(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            window.judgment_analysis_checkbox.setChecked(True)
            ...
            self.assertTrue(
                saved.value("judgment/analyzeAfterConversion", type=bool)
            )
    ...
    def test_ignores_missing_or_unavailable_saved_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            settings.setValue("judgment/analyzeAfterConversion", "not-a-bool")
            ...
            self.assertTrue(window.searchable_checkbox.isChecked())
            self.assertFalse(window.judgment_analysis_checkbox.isChecked())
